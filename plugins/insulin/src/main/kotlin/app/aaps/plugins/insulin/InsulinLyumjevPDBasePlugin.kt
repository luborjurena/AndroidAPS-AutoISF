...
abstract class InsulinLyumjevPDBasePlugin(
    ...
    override val glucodynamic = true

    /**
     * The model fixes the end of action at 8 h and adjusts the shape of the curve to the bolus
     * size, so the DIA of the profile is ignored and the model duration is used everywhere
     * (IOB window, profile switches, Nightscout).
     */
    override val userDefinedDia: Double
        get() = END_OF_ACTION / 60.0
     ...
     * Profile switches created while this plugin is active carry the model DIA, so this can only
     * happen with a profile switch that predates enabling the model.
     */
